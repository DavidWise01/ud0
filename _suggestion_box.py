...
SUGGESTIONS = [
 ...
 dict(id="PS-004", raised="2026-08-18", target="World V / I-13 compiler", world="V", kind="feature",
   status="open", raised_by="darts 053 knapsack, 054 Smith-Waterman, 064 Hungarian, 068 PageRank, 080 Viterbi, 081 Strassen, 085 Gray-Scott",
   title="A 2-D array (matrix)",
   detail="The full knapsack DP is a 2-D table; the corpus array is 1-D, so the dart uses the space-optimized single-row form (which is what practitioners use anyway). Worth considering: a genuine 2-D array, or arrays-of-array-handles, would let the textbook 2-D recurrences run verbatim. Three more darts land here: the Viterbi trellis (080, states x time), Strassen's matrix product (081), and the Gray-Scott grid (085) -- all run flattened into a 1-D array with index arithmetic, but a genuine 2-D array would let their textbook recurrences read verbatim. Now the most-named want after multiple-return. Low urgency -- the 1-D form is honest and correct."),
