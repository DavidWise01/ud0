...
SUGGESTIONS = [
 ...
 dict(id="PS-004", raised="2026-08-18", target="World V / I-13 compiler", world="V", kind="feature",
   status="open", raised_by="darts 053 knapsack, 054 SW, 064 Hungarian, 068 PageRank, 080 Viterbi, 081 Strassen, 085 Gray-Scott, 087 Prim, 089 Householder, 091 marching-cubes, 093 CYK, 094 backprop, 095 Hopfield, 096 Floyd-Warshall, 103 QR-eigenvalue, 104 simplex, 109 Jacobi-eigenvalue",
   title="A 2-D array (matrix)",
   detail="The full knapsack DP is a 2-D table; the corpus array is 1-D, so the dart uses the space-optimized single-row form (which is what practitioners use anyway). Worth considering: a genuine 2-D array, or arrays-of-array-handles, would let the textbook 2-D recurrences run verbatim. Three more darts land here: the Viterbi trellis (080, states x time), Strassen's matrix product (081), and the Gray-Scott grid (085) -- all run flattened into a 1-D array with index arithmetic, but a genuine 2-D array would let their textbook recurrences read verbatim. Now BY FAR the most-named want (17 darts). Batch 17 adds four: Prim's weight matrix (087), Householder QR (089), the 256x16 marching-cubes table (091), and the CYK triangular parse table (093) -- all flattened to 1-D with index arithmetic, all textbook-2-D in the source. Low urgency -- the 1-D form is honest and correct -- but the signal is now overwhelming."),
